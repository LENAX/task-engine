...
func TestTaskBuilder_Basic(t *testing.T) {
	...
	builder := NewTaskBuilderWithRegistry("test-task", "测试任务", registry)

	builder = builder.WithJobFunction("readFile", map[string]interface{}{"fileName": "test.txt"})
...
func TestTaskBuilder_WithDependency(t *testing.T) {
	...
	builder := NewTaskBuilderWithRegistry("task2", "任务2", registry)

	builder = builder.WithJobFunction("httpRequest", map[string]interface{}{"url": "https://www.baidu.com"})
...
func TestTaskBuilder_WithDependencies(t *testing.T) {
	...
	builder := NewTaskBuilderWithRegistry("task3", "任务3", registry)

	builder = builder.WithJobFunction("dbQuery", map[string]interface{}{"query": "SELECT * FROM users"})
...
func TestTaskBuilder_EmptyName(t *testing.T) {
	registry := task.NewFunctionRegistry(nil, nil)
	builder := NewTaskBuilderWithRegistry("", "描述", registry)

	builder = builder.WithJobFunction("func", nil)
	// WithJobFunction 不再返回错误，校验延迟到 Build() 时进行
	// Build 应该返回错误（因为函数未注册）
	_, err := builder.Build()
	if err == nil {
...
func TestTaskBuilder_DefaultValues(t *testing.T) {
	...
	builder := NewTaskBuilderWithRegistry("task", "描述", registry)

	builder = builder.WithJobFunction("writeFile", nil)
...
func TestTaskBuilder_WithTaskHandler_Logging(t *testing.T) {
	...
	builder := NewTaskBuilderWithRegistry("download-task", "下载任务", registry)

	builder = builder.WithJobFunction("downloadFile", map[string]interface{}{"url": "https://example.com/file.zip"})
	...
	taskInstance, err := builder.Build()
	if err != nil {
		t.Fatalf("构建Task失败: %v", err)
	}
...
func TestTaskBuilder_WithTaskHandler_Database(t *testing.T) {
	...
	// 创建TaskBuilder并设置Handler
	builder := NewTaskBuilderWithRegistry("process-task", "处理任务", registry)

	builder = builder.WithJobFunction("processData", map[string]interface{}{"data": "test data"})
...
func TestTaskBuilder_WithTaskHandler_Combined(t *testing.T) {
	...
	builder := NewTaskBuilderWithRegistry("upload-task", "上传任务", registry)

	builder = builder.WithJobFunction("uploadFile", map[string]interface{}{"filePath": "/tmp/file.txt"})

	// 成功时同时使用日志和数据库Handler
	builder = builder.
		WithTaskHandler(task.TaskStatusSuccess, "logHandler").
		WithTaskHandler(task.TaskStatusSuccess, "dbHandler").
		WithTaskHandler(task.TaskStatusFailed, "logHandler")
