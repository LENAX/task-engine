...
funcfindTaskIDByNameOptimized(namestring) string {

iftaskIDValue, exists:=m.workflow.TaskNameIndex.Load(name); exists {

returntaskIDValue.(string)  // O(1)
...
for_, depName:=rangedeps {

depTaskID:=m.findTaskIDByName(depName)  // 重复逻辑

if_, processed:=m.processedNodes.Load(depTaskID); !processed {
...
for_, depName:=ranget.GetDependencies() {
...
funcgetAvailableTasks() []workflow.Task {
...
iftask, exists:=m.workflow.GetTasks()[taskID]; exists {
...
func (m *WorkflowInstanceManager) findTaskIDByName(namestring) string {
